...
- Baseline 2: baseline zero-shot with a structured prompt (run manually, save results)
...
> Run `evaluate.py` on held-out test set. Run keyword-matching baseline on same set. Run baseline zero-shot on 200 test samples (cost: ~$2). Write comparison table.
