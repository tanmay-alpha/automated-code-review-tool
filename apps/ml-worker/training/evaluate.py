...
(from label_mapper.py). A placeholder row for the baseline baseline is
...
# ----------------------------------------------------------------------
# baseline zero-shot baseline (Methodology reference)
# ----------------------------------------------------------------------
def gpt4_baseline_methodology(records: list[dict]) -> None:
    """
    Documenting the methodology used to prompt baseline zero-shot.
...
def _format_md_table(model_per_label: dict, model_summary: dict,
    ...
    Rows:     fine-tuned, keyword, baseline (placeholder).
    """
    lines: list[str] = []
    ...
    lines.append("| baseline zero-shot | _(fill in manually)_ | _(fill in manually)_ | _(fill in manually)_ | _(fill in manually)_ |")
    lines.append("")
    lines.append("## Notes")
    lines.append("")
    lines.append("- baseline baseline row is a placeholder. Run baseline zero-shot on a 200-sample subset of this test set manually and fill in the values.")
    lines.append("- Acceptance per the plan: fine-tuned model macro-F1 must beat both the keyword baseline and the baseline baseline before shipping.")
    lines.append("")
    return "\n".join(lines)
...
def write_results(
    ...
    payload = {
        ...
        "gpt4o_baseline": {
            "note": "Fill in manually — run baseline zero-shot on a 200-sample subset and add the metrics here.",
        },
    }
