...
ALTER TABLE code_samples ADD COLUMN IF NOT EXISTS repository_visibility VARCHAR(20) NOT NULL DEFAULT 'private';
ALTER TABLE code_samples ADD COLUMN IF NOT EXISTS license_spdx VARCHAR(30);
ALTER TABLE code_samples ADD COLUMN IF NOT EXISTS data_use_status VARCHAR(40) NOT NULL DEFAULT 'quarantined_unknown_license';
ALTER TABLE code_samples ADD COLUMN IF NOT EXISTS source_url TEXT;

CREATE INDEX IF NOT EXISTS idx_repos_data_use ON repositories(data_use_status);
CREATE INDEX IF NOT EXISTS idx_code_samples_data_use ON code_samples(data_use_status);
