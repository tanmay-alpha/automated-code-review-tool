...
3. **baseline zero-shot** — manually run on 200 stratified test samples with a
...
| baseline baseline (200 samples) | pending — run evaluate.py | pending — run evaluate.py | pending — run evaluate.py |
...
6. Read `training/data/eval/gpt4o_results.json` for the baseline row.
